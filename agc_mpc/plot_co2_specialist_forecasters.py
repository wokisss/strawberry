# -*- coding: utf-8 -*-
"""Generate standard forecast figures for trained standalone CO2 specialist forecasters."""
...
from benchmark_co2_specialist_forecasters import MODEL_REGISTRY
...
from figure_layout import co2_specialist_figures_dir
...
MODEL_BUILDERS = {
    "co2_env_lstm": ConditionalCO2LSTMForecaster,
    "co2_vmd_lstm_fusion": ConditionalCO2VMDLSTMFusionForecaster,
    "co2_wavelet_gru_attn": ConditionalCO2WaveletGRUAttnForecaster,
}
...
def _plot_one(model_name: str, cfg: AGCConfig, bundle, device: torch.device, regime: str, target_compartment: str) -> None:
    ...
    result = evaluator.evaluate(
        ...
        model_name=run_name,
        output_dir=co2_specialist_figures_dir(cfg.forecast_figures_dir),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        default="all",
        choices=["all", *MODEL_BUILDERS.keys()],
        help="CO2 specialist variant to plot.",
    )
    parser.add_argument(
        "--regime",
        default="joint_all",
...
def main() -> None:
    ...
    for model_name in model_names:
        print("\n" + "=" * 72)
        print(f"Plot CO2 specialist variant: {model_name}")
        print("=" * 72)
