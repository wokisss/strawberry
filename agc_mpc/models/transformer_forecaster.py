# -*- coding: utf-8 -*-
"""Pure Transformer conditional baseline for AGC multi-step forecasting."""
...
class ConditionalTransformerForecaster(nn.Module):
    """Plain Transformer encoder-decoder without hybrid context fusion."""
    ...
    def forward(self, x_past: torch.Tensor, w_future: torch.Tensor, u_future: torch.Tensor) -> torch.Tensor:
        memory = self.encoder(self.pos_past(self.past_proj(x_past)))
        future_tokens = self.pos_future(self.future_proj(torch.cat([w_future, u_future], dim=-1)))
